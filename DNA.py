    ...
    def extract_and_reverse_compression(self):
        if namez == "e":
            ...
            while len(compressed_data) > 0:
                code = compressed_data[0:2]
                compressed_data = compressed_data[2:]

                if code == "00":
                    # No compression
                    uncompressed_data += compressed_data[0:8]
                    compressed_data = compressed_data[8:]
                elif code == "01":
                    # Compression
                    run_length = int(compressed_data[0:2], 2)
                    uncompressed_data += compressed_data[2:7]
                    compressed_data = compressed_data[7:]
            ...
            with open(output_file, "wb") as extracted_file:
                extracted_file.write(binascii.unhexlify(uncompressed_data))

            print(f"Extraction complete. Saved as {output_file}")
